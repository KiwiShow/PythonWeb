...
class MonModel(object):
    ...
    def _find(cls, **kwargs):
        ...
        ds = mon.web_flask[name].find(kwargs)
        if sort is not None:
            ds = ds.sort(sort)
    ...
    def remove(cls, id):
        ...
        mon.web_flask[name].update_one(query, {"$set": values})

    def save(self):
        name = self.__class__.__name__
        mon.web_flask[name].save(self.__dict__)
    ...
    def data_count(self, cls):
        ...
        count = mon.web_flask[name]._find(query).count()
        return count
